...
public class XLSX2CSV {
	...
	Map<Integer, String> cellDataMap = new HashMap<Integer, String>();	//excel表格中每列值集合（key为列序号，value为列值）
	Map<Integer, Map<Integer, String>> rowDataMap = new HashMap<Integer, Map<Integer, String>>();	//excel表格中每行数据计划（key为行序号，value为列值集合）
	...
	class MyXSSFSheetHandler extends DefaultHandler {
		...
		public void endElement(String uri, String localName, String name)
			...
			if ("v".equals(name)) {
				...
				cellDataMap.put(thisColumn, thisStr.replaceAll("[\\t\\n\\r]", ""));
				...
				if (rowNo == 0) {
					rowDataMap = new HashMap<Integer, Map<Integer, String>>();
				}
				
				rowDataMap.put(rowNo, cellDataMap);
				rowNo++;
				cellDataMap = new HashMap<Integer, String>();
	 ...
	 * @throws SAXException
	 */
	public Map<Integer, Map<Integer, String>> process() throws IOException, OpenXML4JException,
		...
		//修改为只读取第一个Sheet页,以后读取多个Sheet页通过此处扩展
		int index = 0;
	...
	public static void main(String[] args) throws Exception {
		OPCPackage p = OPCPackage
				.open("D:/workspace/makeplays/src/main/webapp/template/viewImportTemplate.xlsx",
						PackageAccess.READ);
		XLSX2CSV xlsx2csv = new XLSX2CSV(p, System.out, 12);
		Map<Integer, Map<Integer, String>> resultMap = xlsx2csv.process();
	...
	/**
	 * 读取xlsx数据，以Map<Integer, Map<Integer, String>>形式返回结果，
	 * 最外层Map中键为行号，值为列数据集合
	 * 里层Map中键为列号，值为列值，如果表格中其中某一行的某一列为空，则列值集合中不会有该列号
	 * @param filepath	文件路径
	 * @param minColumnNum	表格最小列数
	 * @return
	 * @throws Exception
	 */
	public static Map<Integer, Map<Integer, String>> readXlsxData(String filepath, int minColumnNum) throws Exception {
		OPCPackage p = OPCPackage.open(filepath, PackageAccess.READ);
		XLSX2CSV xlsx2csv = new XLSX2CSV(p, System.out, minColumnNum);
		Map<Integer, Map<Integer, String>> resultMap = xlsx2csv.process();
		return resultMap;
	}
