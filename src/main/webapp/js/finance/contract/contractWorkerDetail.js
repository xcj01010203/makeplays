...
function selectWorkerName(own){
	...
	$.ajax({
		...
		success: function(response){
			if(response.success){
				var workerNameList = response.workerNameList;
				if(workerNameList.length != 0){
					own.next("ul.dropdown_box").find("li").remove();
					$.each(workerNameList, function(i,value){
						own.next("ul.dropdown_box").append("<li class='drop-down-li' title='"+ value +"'><a href='javascript:void(0)' onclick='getContactDuty(this)'>" + value + "</a></li>");
					});
					own.next("ul.dropdown_box").css({left: own.position().left, top: own.position().top+own.outerHeight()}).show();
				}
				
				$("#sonWorkerName").keyup(function(){
				 ...
				 $('.dropdown_box').on('click','li',function(ev){
					  var _this=$(this);
					  $('#sonWorkerName').val($(this).text());
					  //清楚错误提示
					  $(".name-error-tips").hide();
					  $("#sonWorkerName").removeClass("worker-name-tips");
					  _this.parent().hide();
				 });
			}else{
...
function checkOutIsEmpty(){
	//名称
	$("#sonWorkerName").on("blur", function(){
		if($(this).val() == ""){
			$(".name-error-tips").show();
			$(this).addClass("worker-name-tips");
		}
	}).on("focus", function(){
		$(".name-error-tips").hide();
		$(this).removeClass("worker-name-tips");
	});
	//总金额
...
function percentCalculation(obj){
	var tatolMoney = $.trim($("#totalMoney").val().replace(/,/g,""));
	
	var totalMoney = $("#totalMoney").next("input[type=hidden]").val();
...
//	initPayMethod();
	...
	$("input[name='payMethod']").on("click", function(){
		...
		$("table.worker-pay-method").find("#byMonthOrByStage tr:not(:first)").remove();//删除 除了第一行的所有数据
		...
		//给定默认日期
		/*$("table.worker-pay-method").find("#byMonthOrByStage tr:first").find("td.remind-time-td").find("input.date-select").attr("onClick","WdatePicker({readOnly:true, isShowClear:false})");
		$("table.worker-pay-method").find("#byMonthOrByStage tr:first").find("td.remind-time-td").find("input.date-select").val("");*/
		$("table.worker-pay-method").find("#byMonthOrByStage tr:first").find("td.payment-td").find("input.date-select").attr("onClick","WdatePicker({readOnly:true, isShowClear:false})");
		$("table.worker-pay-method").find("#byMonthOrByStage tr:first").find("td.payment-td").find("input.date-select").val("");
	});
}
...
function addPayMethodLine(){
	...
	$('.addition').on("click",function(){
		//判断是否选择提醒日期
		/*var remindTime= $(this).parent("div").parent("td").siblings("td.remind-time-td").eq(0).find("input.date-select").eq(0).val();*/
...
//			payWayTotalMoney = add(payWayTotalMoney, multiply(monthMoney, startDate));
		...
		$("#byMonthOrByStage tr").each(function(i){
			...
			/*payWayTotalMoney = parseFloat(payWayTotalMoney)+ parseFloat(money);*/
...
//保存职员信息
function saveWorkerContractInfo(){
	...
	subData.workerName = $("#sonWorkerName").val();
	subData.department = $("#sonDepartment").val();
	...
	subData.enterDate = $("#sonEnterDate").val();
	subData.leaveDate = $("#sonLeaveDate").val();
	subData.currencyId = $("#sonCurrencyId").val();
	var totalMoney = $("#sonTotalMoney").parent('td').find("input[type=hidden]").val();
	...
	if($("#sonWorkerName").val() == ""){
		$(".name-error-tips").show();
		$("#sonWorkerName").addClass("worker-name-tips");
		return;
	}
	if($("#sonCurrencyId").val() == ""){
		$(".currency-error-tips").show();
		$("#sonCurrencyId").addClass("worker-name-tips");
		return;
	}
	if($("#sonTotalMoney").val() == ""){
		$(".total-error-tips").show();
		$("#sonTotalMoney").addClass("worker-name-tips");
		return;
	}
	/*if($("#sonEnterDate").val() == ""){
		$(".el-date-error-tips").show();
		$("#sonEnterDate").addClass("worker-name-tips");
		return;
	}
	if($("#sonLeave").val() == ""){
		$(".el-date-error-tips").show();
	...
	if($("#sonContractDate").val() == ""){
		...
		$("#sonContractDate").addClass("worker-name-tips");
...
//		}
	...
	$.ajax({
		url: '/contractWorker/queryRepeatData',
	    type: 'post',
	    data: {workerName:subData.workerName, phone: subData.phone,contractId: subData.contractId},
	    datatype: 'json',
	    async: false,
	    success: function(response){
	    	if(response.success){
	    		...
	    		if (isRepeat) { //有重复数据
					InfoMessage = "已经保存过 " + subData.workerName + " 的合同信息，是否继续保存？ ";
				}
	    	}else{
	    		parent.showErrorMessage(response.message);
	    	}
...
function saveWorkerInfos(subData){
	$.ajax({
		url: '/contractWorker/saveContractInfo',
	    type: 'post',
	    data: subData,
	    datatype: 'json',
	    success: function(response){
...
function initModifyWorkerInfo(){
	...
	//是否渲染删除按钮
	if($("#contractId").val() != ""){
		$(".right-popup-title").text("修改职员信息");
		//设置权限
		if(!isContractReadonly){
			$(".right-popup-delete").css("display", "block");
		}
		$.ajax({
			url: '/contractWorker/queryContractById',
			type: 'post',
			...
			success: function(response){
				if(response.success){
					...
					$("#sonWorkerName").val(contractInfo.workerName);
					$("#sonDepartment").val(contractInfo.department);
					...
					$("#sonEnterDate").val(contractInfo.enterDate);
					$("#sonLeaveDate").val(contractInfo.leaveDate);
					...
					if(attachmentList !=null || attachmentList.length != 0){
						...
						for(var i= 0; i< attachmentList.length; i++){
							...
							if(suffix == ".doc" || suffix == ".docx"){
								...
								html.push("<li class='upload-file-list-li' onclick='previewAtts(\""+ attachment.attpackId +"\", \""+ attachment.type 
										+"\", \""+ attachment.hdStorePath +"\")'><img src='../images/pdf.jpg' title='"+ attachment.name 
										+"'><a class='download-tag' onclick='downLoadAttachment(event,this,\""+ attachment.id 
										+"\")' title='下载'></a><a class='closeTag' onclick='deleteUploadedFile(event,this,\""+ attachment.id +"\")'></a><p class='file-list-tips' title='"
...
//删除职员合同信息
function deleteWorkerContractInfo(){
	
	parent.popupPromptBox("提示","是否删除该职员合同？", function (){
		var contractId = $("#contractId").val();
		$.ajax({
			url: '/contractWorker/deleteContract',
			type: 'post',
			data: {contractId: contractId},
			datatype: 'json',
			success: function(response){
...
//    var gridWidth;
//    if(screenWidth >= 1366 && screenWidth < 1400){
//    	gridWidth = 600;
//    }
...
//根据填写的姓名自动获取部门职务
function getContactDuty(own){
	var contactName = "";
	//获取填写的姓名
	contactName = $(own).text();
	if (contactName == "") {
		contactName = $(own).val();
	}
	if (contactName != "" && contactName != undefined) {
		//根据姓名获取职务
		$.ajax({
			url: "/contractWorker/queryDepartmentList",
			type: "post",
