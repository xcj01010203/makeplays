		...
		success: function(response){
			if(response.success){
				...
				if(actorNameList.length != 0){
					...
					$.each(actorNameList, function(i,value){
						own.next("ul.dropdown_box").append("<li class='drop-down-li' title='"+ value +"'><a href='javascript:void(0)' onclick='autoAddRoleName(this)'>" + value + "</a></li>");
					});
					own.next("ul.dropdown_box").css({left: own.position().left, top: own.position().top+own.outerHeight()}).show();
				}
				
				$("#sonActorName").keyup(function(){
				 ...
				 $('.dropdown_box').on('click','li',function(ev){
					  var _this=$(this);
					  $('#sonActorName').val($(this).text());
					  //清楚错误提示
					  $(".name-error-tips").hide();
					  $("#sonActorName").removeClass("actor-name-tips");
					  _this.parent().hide();
				 });
			}else{
...
function checkOutIsEmpty(){
	//名称
	$("#sonActorName").on("blur", function(){
		if($(this).val() == ""){
			$(".name-error-tips").show();
			$(this).addClass("actor-name-tips");
		}
	}).on("focus", function(){
		$(".name-error-tips").hide();
		$(this).removeClass("actor-name-tips");
	});
	//总金额
...
//      	 	var text = $("input[name=subval]").val();
	 ...
	 $("#filterBtn").click(function(ev){
		...
		$('#sonFinanceSubjName').click(function(ev){
			var obj = $(this);
	...
	if(selValue == 1){
		if($this.val() != ""){
			...
			$("#byMonthOrByStage tr.byStageTr").each(function(){
				...
				$(this).find("input[type=text]").eq(2).val(fmoney(multiply(divide(payedRange, 100),totalMoney),2));
...
function percentCalculation(obj){
	...
	var tatolMoney = $.trim($("#sonTotalMoney").val().replace(/,/g,""));
	var val = $.trim($(obj).val()).replace(/,/g,"");
	if(tatolMoney !="" && val!=undefined && val != "" && !isNaN(val)){
		$(obj).next("input").val(fmoney(val/100*tatolMoney,2));
	}
...
//	initPayMethod();
	...
	$("input[name='payMethod']").on("click", function(){
		...
		$("#byMonthOrByStage tr:not(:first)").remove();//删除 除了第一行的所有数据
	    var selValue = radioValue;
  		$('.payment-td').html('');
  		var paymentContent = "";
		if(selValue == 1){
			//paymentContent = '<input type="hidden" vlaue="" /><span>第</span><span class="spanClass" id="1">一</span><span>期: <input class="stage-info" type="text" maxlength="180"  onkeyup="setTitle(this);">,甲方向乙方支付酬金总额的<input class="stage-money-part" type="text" onblur="percentCalculation(this);" maxlength="10">% ,即人民币<input class="stage-money" type="text" maxlength="10" onkeyup="checkNum(this);" onblur="moneyFormat(this);" class="stage-money"> 元整。</span><span class="remind-tips">预计付款日期:</span><input class="date-select remind-input" type="text" onClick="WdatePicker({readOnly:true,isShowClear:false})">';
		...
		if(selValue == 2){
  			//paymentContent = '<input type="hidden" value="" /><span><input class="stage-info" type="text" maxlength="180" onkeyup="setTitle(this);" >,甲方向乙方按月支付酬金，月薪为人民币<input type="text" class="month-money stage-money" maxlength="10" onblur="moneyFormat(this);">元整,共<input  class="stage-money-part" onkeyup="checkNum(this)" type="text" maxlength="4">个月。</span><span class="remind-tips">预计付款日期:</span><input class="date-select remind-input" type="text" onClick="WdatePicker({readOnly:true,isShowClear:false})">';
		...
		//给定默认日期
//		$("table.actor-pay-method").find("#byMonthOrByStage tr:first").find("td.payment-td").find("input.date-select").attr("onClick","WdatePicker({readOnly:true, isShowClear:false})");
//		$("table.actor-pay-method").find("#byMonthOrByStage tr:first").find("td.payment-td").find("input.date-select").val("");
	});
}
...
//选择日期后显示title
function pickedFunc(){
	//$(".pay-method-title").attr('title')+$dp.cal.getDateStr()
    	$($dp.$(this)).prev("img").prop("title", "提醒时间\n"+$dp.cal.getDateStr());
}
...
function addPayMethodLine(){
	//判断是否选择支付日期
	var remindTime= $(this).parent("div").parent("td").parent("tr.byStageTr").find("input[type=text].remind-input").eq(0).val();
	if(remindTime == ""){
...
//			_copyTr.find("input.end-date").attr("onClick","WdatePicker({minDate:'"+ newStartdate +"'})");
	...
	$('.subtraction').click(function(){
		var selValue = $("input[name=payMethod]:checked").val();
		var allTr = $("#byMonthOrByStage tr.byStageTr");
		if(allTr.length == 1){
...
//保存演员信息、
function saveActorContractInfo(){
	var subData = {};
	...
	subData.actorName = $("#sonActorName").val();
	subData.roleName = $("#sonRoleName").val();
	subData.phone = $("#sonPhone").val();
	/*subData.idNumber = $("#sonIdNumber").val();*/
	...
	if($("#sonActorName").val() == ""){
		$(".name-error-tips").show();
		$("#sonActorName").addClass("actor-name-tips");
		return;
	}
	if($("#sonCurrencyId").val() == ""){
		$(".currency-error-tips").show();
		$("#sonCurrencyId").addClass("actor-name-tips");
		return;
	}
	if($("#sonTotalMoney").val() == ""){
		$(".total-error-tips").show();
		$("#sonTotalMoney").addClass("actor-name-tips");
		return;
	}
	if($("#sonContractDate").val() == ""){
		$(".contract-error-tips").show();
		$("#sonContractDate").addClass("actor-name-tips");
	...
	if(objPayMentInfo.paymentTerm == undefined){
		return;
	}
	var payTotalMoney = 0;
...
//		}
	...
	$.ajax({
		url: '/contractActor/queryRepeatData',
	    type: 'post',
	    data: {actorName:subData.actorName, phone: subData.phone, contractId: subData.contractId},
	    datatype: 'json',
	    async: false,
	    success: function(response){
	    	if(response.success){
	    		...
	    		if (isRepeat) { //有重复数据
					InfoMessage = "已经保存过 " + subData.actorName + " 的合同信息，是否继续保存？ ";
				}
	    	}else{
	    		parent.showErrorMessage(response.message);
	    	}
	...
	if(payTotalMoney != totalMoney-0){
		InfoMessage += "支付方式金额合计与总金额不相等，是否保存？";
	}	
	
	if (InfoMessage != "" && InfoMessage.length>0) {
		parent.popupPromptBox("提示",InfoMessage, function (){
...
function saveActorInfos(subData){
	$.ajax({
		url: '/contractActor/saveContractInfo',
		type: 'post',
		data: subData,
		datatype: 'json',
		success: function(response){
...
function initModifyActorInfo(){
	...
	//是否渲染删除按钮
	if($("#contractId").val() != ""){
		$(".right-popup-title").text("修改演员合同");
		if(!isContractReadonly){
			$(".right-popup-delete").css("display", "block");
		}
		$.ajax({
			url: '/contractActor/queryContractById',
			type: 'post',
			...
			success: function(response){
				if(response.success){
					...
					$("#sonActorName").val(contractInfo.actorName);
					$("#sonRoleName").val(contractInfo.roleName);
...
//									_copyTr.find(".date-select").attr("onClick","WdatePicker({readOnly:true,isShowClear:false})");
						...
						if(contractMonthPayWayList.length == 0){
							...
							paymentContent += '<input type="hidden" value="" />';
				  			...
				  			paymentContent += '    <input class="month-money input-middle" type="text" maxlength="10" onblur="checkIsMoney(this)">元，付款周期为';
				  			paymentContent += '    <input class="start-date input-middle" type="text" onClick="WdatePicker({readOnly:true,isShowClear:false})">至';
				  			paymentContent += '    <input class="end-date input-middle" type="text" onClick="WdatePicker({readOnly:true,isShowClear:false})">结算日为每日';
				  			paymentContent += '    <input class="month-pay-day input-small" type="text" maxlength="2" onblur="checkMonthDayInput(this)">日';
				  			paymentContent += '</span>';
						...
						for(var i= 0; i< attachmentList.length; i++){
							...
							if(suffix == ".doc" || suffix == ".docx"){
								html.push("<li class='upload-file-list-li'  onclick='previewAtts(\""+ attachment.attpackId +"\", \""
										+ attachment.type +"\", \""+ attachment.hdStorePath +"\")'><img src='../images/word.jpg' title='"
...
//								html.join("");
								html.push("<li class='upload-file-list-li'  onclick='previewAtts(\""+ attachment.attpackId +"\", \""
										+ attachment.type +"\")'><img src='/fileManager/previewAttachment?address="+attachment.hdStorePath+"' title='"
										+ attachment.name +"'><a class='download-tag' onclick='downLoadAttachment(event,this,\""+ attachment.id 
										+"\")' title='下载'></a><a class='closeTag' onclick='deleteUploadedFile(event,this,\""+ attachment.id 
										+"\")'></a><p class='file-list-tips' title='"+ attachment.name +"'>" + attachment.name + "</p></li>");
...
//删除演员合同
function deleteActorContractInfo(){
	parent.popupPromptBox("提示","是否删除该演员合同？", function (){
		var contractId = $("#contractId").val();
		$.ajax({
			url: '/contractActor/deleteContract',
			type: 'post',
			data: {contractId: contractId},
			datatype: 'json',
			success: function(response){
...
function autoAddRoleName(own){
	...
	if (actorName != "" && actorName != undefined) {
		$.ajax({
			url: "/contractActor/queryViewRoleName",
			type: "post",
			...
			success: function(response) {
				if (!response.success) {
					...
					var viewRoleName = response.viewRoleName;
					$("#sonRoleName").val(viewRoleName);
				}
			}
		});
	}
